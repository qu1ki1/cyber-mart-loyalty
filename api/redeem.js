...
export default async function handler(req, res) {
  ...
  try {
    const { data: business } = await supabase
      .from('businesses')
      .select('id, owner_password, staff_password, manager_password')
      .ilike('slug', slug)
      .maybeSingle()

    const validPasswords = [business?.owner_password, business?.staff_password, business?.manager_password].filter(Boolean)
