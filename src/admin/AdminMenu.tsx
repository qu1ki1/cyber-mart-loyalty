...
export default function AdminMenu() {
  ...
  return (
    <div className="admin-header">
      ...
      <nav>
        {items.map((item) => (
          <a key={item.url} href={item.url} className={path === item.url ? 'active' : ''}>
            {item.icon}
            <span>{item.name}</span>
          </a>
        ))}
      ...
      <style>{`
        ...
        nav span{ font-size:10px; letter-spacing:.1em; }
        nav a:hover{ transform: translateY(-3px); color:white; }
        nav a.active{
          color:var(--neon);
          background: rgba(57,255,138,.08);
          border-color: var(--line);
          box-shadow: 0 0 25px rgba(57,255,138,.25);
        }
        @media(max-width:700px){
          .admin-header{ flex-direction:column; gap:15px; }
          nav{ width:100%; justify-content:space-between; }
          nav a{ width:55px; height:55px; }
